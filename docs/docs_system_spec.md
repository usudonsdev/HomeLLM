...
- **検討中**: 1 時間級 VOD のクライアントアップロードでは Tailscale がボトルネックになりうるため、大容量転送向きの経路への置き換え／併用を検討する（ADR-008 補足）
...
フロント（`services/web`）は **静的エクスポート**し、デスクトップから SSH で Pi に配置する（`scripts/deploy-web-pi.ps1`）。ブラウザは Tailscale 経由で **job-hunting-api** と **video-ingest-api** を別々に呼ぶ。動画バイナリは **クライアント → Windows API 直送**（Pi はプロキシしない。ADR-008）。
