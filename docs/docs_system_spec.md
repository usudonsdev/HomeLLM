...
[ Windows + k3s ] … 必要時起動
  ...
  ├── Namespace video-analysis: ingest API + Valorant Jobs
  └── [ホスト] Ollama qwen3.5:9b :11434
```

フロント（`services/web`）は **静的エクスポート**し、デスクトップから SSH で Pi に配置する（`scripts/deploy-web-pi.ps1`）。ブラウザは Tailscale 経由で Windows API を直接呼ぶ（動画バイナリは UI 経由で送らない）。
